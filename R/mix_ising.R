...
##' @author n.stadler
##' @keywords internal
MStepIsing <- function(x,chromosome=NULL,u,v=NULL,lambda,gamma,pen,penalize.diagonal,equal.prob.trans=NULL,term,
                       Mu_old, Sig_old){
  n.comp <- ncol(u)
  n <- dim(x)[1]
  p <- dim(x)[2]
  ...
  for (l in 1:n.comp){
    ...
    rho=2*(pi.comps^{gamma})*lambda/(sum(u[,l]))
    
    # n_one = t(x) %*%u[,l,drop=FALSE]
    # to_fit = which(n_one > 1e-3)
    # not_fit = which(n_one <= 1e-3)
    
    fit.ising <- IsingFit(x, sample_weights=u[,l], progressbar=FALSE, plot=FALSE)
    Sig[,,l] <- fit.ising$weiadj
    Mu[,l] = fit.ising$thresholds
    
    # if(length(not_fit) > 0) {
    #   cat('engaged')
    #   Sig[not_fit,,l] = Sig_old[not_fit,,l]
    #   Sig[,not_fit,l] = Sig_old[,not_fit,l]
    #   Mu[not_fit,l] = Mu_old[not_fit,l]
    # }
  }
  
  return(list(mix.prob=mix.prob,Sig=Sig, Mu=Mu))
}
...
mixglasso_init<- function(x,n.comp,lambda,
  ...
  if (n.comp==1){
    ...
    for (l in 1:n.comp){
      ...
      rho=2*(pi.comps^{gamma})*lambda/(sum(u[,l]))
      fit.ising <- IsingFit(x, sample_weights=u[,l], gamma=gamma, progressbar=FALSE, plot=FALSE)
      Sig[,,l] <- fit.ising$weiadj
      Mu[,l] = fit.ising$thresholds 
    ...
    while((err1>term)&(iter<maxiter)){
      ...
      for (l in 1:n.comp){
        logphi[,l] <- isingLL(x,Sig[,,l],Mu[,l])
      }
      
      fit.E <- EXPStep.mix(logphi,mix.prob)
      unew <- fit.E$u
      
      n_one = sapply(1:n.comp, function(l) any(t(x) %*%unew[,l,drop=FALSE] < 1e-3))
      print(n_one)
      
      loglik <- fit.E$loglik
      
      if (show.trace==TRUE){
        cat('         -mixglasso: loglik',loglik,'\n')
      }
      

      if (((any(colSums(unew)<=min.compsize))&(iter>miniter))|any(colSums(unew)<=5)|any(n_one)){#min.compsize is a tuning-param for stoping EM; EM stops always if compsize<5
        compsize <- colSums(u)
      ...
      ##Mstep
      fit.M <- MStepIsing(x=x,u=u,lambda=lambda,gamma=gamma,pen=pen,penalize.diagonal=penalize.diagonal,term=term,
    ...
    n.zero <- sum(SigInv==0)
    compsize <- colSums(u)
    DF <- (n.comp-1)+n.comp*(p*(p+1)/2)-n.zero/2#df.mean+df.mix.prob+df.inv.covariance
    n.zero.percomp <- apply(SigInv==0,3,sum)
    penmmdl <- log(n)*(n.comp-1)+sum(log(compsize)*((p*(p+1)/2)-n.zero.percomp/2))
...
# Log likelihood for Ising model

isingLL <- function(x, weighted_adj, thresholds) {
  nvar = ncol(x)
  N <- nrow(x)
  
  logl_M <- P_M <- matrix(0, N, nvar)
  
  for (i in 1:nvar){  
    betas.ii <- weighted_adj[-i,i]
    int.ii <- thresholds[i]
    y <- matrix( 0 , nrow=N , ncol= 1 ) 
    xi <- x[,-i]
    NB <- length( betas.ii) # number of rows in beta
    for (bb in 1:NB){   
      y <- y + betas.ii[rep(bb,N)] * xi[,bb]
    }
    y <- matrix( int.ii , nrow=N , ncol=1 , byrow=TRUE ) + y
    
    # calculate P matrix
    P_M[,i] <- exp(y*x[,i])/(1+exp(y))
    logl_M[,i] <- log(P_M[,i])  
  }
  return(rowSums(logl_M))
}
